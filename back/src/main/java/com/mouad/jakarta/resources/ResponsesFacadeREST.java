...
@Stateless
@Path("responses")
public class ResponsesFacadeREST extends AbstractFacade<Responses> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Integer id, Responses entity) {
        super.edit(entity);
    }
    ...
    public Responses find(@PathParam("id") Integer id) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Responses> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
